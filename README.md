...
![Python](https://img.shields.io/badge/python-3.10%20·%203.11%20·%203.12-cyan?style=flat-square&logo=python&logoColor=white)
![TensorFlow](https://img.shields.io/badge/tensorflow-2.18-cyan?style=flat-square&logo=tensorflow&logoColor=white)
...
![Version](https://img.shields.io/badge/version-2.1.0-magenta?style=flat-square)
...
- **Keras 3 native SynthSeg port** — TensorFlow 2.18 + Keras 3 on Python 3.10–3.12 (no legacy Keras shim).
- **Adaptive acceleration** — NVIDIA CUDA gets XLA + mixed precision (`float16`); Apple Metal and CPU stay on eager FP32 with automatic fallback if a compiled path fails.
- **Lower peak RAM** — QC runs as a separate model pass; pad buckets (160/192/224/256) reduce wasted padding; posteriors stored as `float16` after argmax.
- **CLI / env controls** — `--no-accel` disables XLA and mixed precision; `NEUROFLUX_NO_ACCEL=1` and `NEUROFLUX_XLA=0|1` override the policy.
- **`GET /gpu`** — now reports `accel`, `dtype`, `accelerator`, and `bucket_padding` alongside GPU name.
...
**Options:** `--robust` (clinical / low-SNR scans) · `--fast` (~2× speed) · `--ct` (CT Hounsfield mode) · `--threads N` · `--low-memory` (6–8 GB systems) · `--no-accel` (eager FP32 only)
...
│  · Adaptive accel (accel.py) — XLA/FP16 on NVIDIA       │
│  · Apple Silicon GPU via tensorflow-metal 1.2 (TF 2.18)  │
...
| Env | Python | Purpose |
|-----|--------|---------|
| `neuro_venv` | 3.10 / 3.11 / 3.12 | Everything: Flask · SynthSeg · TensorFlow 2.18 |
...
# Install NeuroFlux (includes SynthSeg + TensorFlow 2.18)
uv pip install -e .
...
```bash
neuroflux-segment path/to/scan.nii.gz [output_dir] [--robust] [--fast] [--threads 4] [--no-accel]
```
...
| `GET` | `/gpu` | GPU + acceleration probe (`accel`, `dtype`, `accelerator`, `bucket_padding`) |
...
**Single environment (Python 3.10–3.12)**

| Package | Purpose |
|---------|---------|
| `tensorflow 2.18` | SynthSeg inference (bundled, Keras 3) |
...
|-------|---------|---------|
| `[metal]` | `tensorflow-metal==1.2.0` (pins TF 2.18) | Apple Silicon GPU |

---

## ⬡ KNOWN ISSUES & NOTES

- **Python 3.13+:** Not supported yet — TF/Metal wheels for 3.13 are not stable. Use Python 3.10–3.12.
- **Acceleration:** NVIDIA CUDA benefits most from XLA + mixed precision (enabled automatically). Metal and CPU use eager FP32; RAM optimizations (QC split, pad buckets, float16 posteriors) apply on all platforms. Set `NEUROFLUX_NO_ACCEL=1` or pass `--no-accel` to force baseline eager FP32.
- **Apple Silicon:** install `pip install "neuroflux[metal]"` for GPU acceleration (requires TF 2.18 pin). Without it, inference runs on CPU (~1 min/scan).
...
- **Windows GPU:** TF 2.18 supports CUDA on Windows. For CPU-only use, no extra setup is needed.
