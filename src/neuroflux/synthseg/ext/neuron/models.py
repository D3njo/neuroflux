...
from . import layers
from ...keras_compat import model_input_tensor, model_output_tensor, tensor_shape_list, tensor_static_dim
...
def _batch_norm_layer(axis, name):
    ...
    try:
        ...
        # Keras 3 rejects the fused= kwarg (ValueError); Keras 2 rejects unknown kwargs (TypeError).
...
def ae(nb_features,
    ...
    if single_model:
        ...
        in_input_shape = tensor_shape_list(model_output_tensor(enc_model))[1:]
        in_model = None
    ...
    if single_model:
        ...
        in_input_shape = tensor_shape_list(model_output_tensor(mid_ae_model))[1:]
        in_model = None
    dec_model = conv_dec(nb_features,
                         in_input_shape,
                         nb_levels,
...
def conv_enc(nb_features,
    ...
    for level in range(nb_levels):
        ...
        if use_residuals:
            ...
            nb_feats_in = tensor_static_dim(lvl_first_tensor, -1)
            nb_feats_out = tensor_static_dim(convarm_layer, -1)
...
def conv_dec(nb_features,
    ...
    if input_model is None:
        ...
        input_tensor = model_input_tensor(input_model)
        last_tensor = model_output_tensor(input_model)
        input_shape = tensor_shape_list(last_tensor)[1:]
    ...
    for level in range(nb_levels - 1):
        ...
        if use_residuals:
            ...
            # However, it may not have the right number of features to be added
            add_layer = up_tensor
            nb_feats_in = tensor_static_dim(add_layer, -1)
            nb_feats_out = tensor_static_dim(last_tensor, -1)
            if nb_feats_in > 1 and nb_feats_out > 1 and (nb_feats_in != nb_feats_out):
...
def single_ae(enc_size,
    ...
    if input_model is None:
        ...
        last_tensor = model_output_tensor(input_model)
        input_shape = tensor_shape_list(last_tensor)[1:]
    input_nb_feats = tensor_shape_list(last_tensor)[-1]
    ...
    if ae_type == 'dense':
        ...
        if list(enc_size)[:-1] != list(input_shape)[:-1] and \
            ...
            zf = [enc_size[:-1][f] / tensor_shape_list(last_tensor)[1:-1][f] for f in range(len(enc_size) - 1)]
    ...
    if do_vae:
        ...
        if ae_type == 'dense':
            ...
            if list(enc_size)[:-1] != list(input_shape)[:-1] and \
                ...
                name = '%s_ae_sigma_enc' % prefix
                last_tensor = convL(tensor_shape_list(pre_enc_layer)[-1], conv_size, name=name, **conv_kwargs)(
                    pre_enc_layer)
    ...
    if ae_type == 'dense':
        ...
        if list(enc_size)[:-1] != list(input_shape)[:-1] and \
            ...
            zf = [tensor_shape_list(last_tensor)[1:-1][f] / enc_size[:-1][f] for f in range(len(enc_size) - 1)]
            last_tensor = layers.Resize(zoom_factor=zf, name=name)(last_tensor)
