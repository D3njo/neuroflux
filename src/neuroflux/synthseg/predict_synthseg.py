...
# project imports
from neuroflux.accel import bucket_pad_shape, get_compiled_predictor, get_policy
...
@dataclass
class SynthSegModelBundle:
    """Segmentation network plus optional standalone QC models."""

    seg_net: Model
    qc_net: Model | None = None
    qc_prep: Model | None = None

# third-party imports
from .ext.lab2im import utils
from .ext.lab2im import layers
from .ext.lab2im import edit_volumes
from .keras_compat import set_keras_shape, tensor_static_shape
from .ext.neuron import models as nrn_models


def _run_inference(bundle, image, shape_input, do_parcellation, do_qc, bucket_shape):
    """Run segmentation (and optional QC) with adaptive XLA/eager policy."""
    seg_predict = get_compiled_predictor(bundle.seg_net, bucket_shape)

    if do_parcellation:
        seg_out = seg_predict(image)
        if isinstance(seg_out, tuple):
            post_patch_segmentation, post_patch_parcellation = seg_out[0], seg_out[1]
        else:
            post_patch_segmentation, post_patch_parcellation = seg_out, None
    else:
        seg_out = seg_predict(image)
        if isinstance(seg_out, tuple):
            post_patch_segmentation = seg_out[0]
        else:
            post_patch_segmentation = seg_out
        post_patch_parcellation = None

    qc_score = None
    if do_qc and bundle.qc_net is not None and bundle.qc_prep is not None:
        qc_volume = _prepare_qc_input(post_patch_segmentation, shape_input, bundle.qc_prep)
        qc_score = _run_qc_network(bundle.qc_net, qc_volume)

    return post_patch_segmentation, post_patch_parcellation, qc_score


def _prepare_qc_input(post_patch_seg, shape_input, qc_prep_model):
    hard = np.argmax(post_patch_seg, axis=-1).astype(np.int32)
    out = qc_prep_model([hard, shape_input], training=False)
    return out.numpy() if hasattr(out, "numpy") else np.asarray(out)


def _run_qc_network(qc_net, qc_volume):
    bucket = tuple(qc_volume.shape[1:4]) if get_policy().use_xla else None
    fn = get_compiled_predictor(qc_net, bucket)
    out = fn(qc_volume)
...
def _model_predict(net, inputs):
    ...
    On TensorFlow 2.18 + Keras 3 (and tensorflow-metal), Keras's compiled predict()
...
def predict(path_images,
    ...
    if _cache_key in _MODEL_CACHE:
        bundle = _MODEL_CACHE[_cache_key]
    else:
        bundle = build_model(path_model_segmentation=path_model_segmentation,
                             ...
                             do_qc=do_qc)
        _MODEL_CACHE[_cache_key] = bundle
    ...
    for i in range(len(path_images)):
        ...
        if compute[i]:
            ...
            try:
                ...
                bucket_shape = tuple(image.shape[1:4])
                shape_input = utils.add_axis(np.array(image.shape[1:-1]))
                post_patch_segmentation, post_patch_parcellation, qc_score = _run_inference(
                    bundle,
                    image=image,
...
def preprocess(path_image, ct, target_res=1., n_levels=5, crop=None, min_pad=None, path_resample=None):
    ...
    # pad image — bucket shapes stabilise XLA graphs and cap padding waste
    input_shape = im.shape[:n_dims]
    min_pad = utils.reformat_to_list(min_pad, length=n_dims, dtype='int')
    if get_policy().use_bucket_padding:
        pad_shape = bucket_pad_shape(input_shape, min_pad=min_pad)
    else:
        pad_shape = [
            utils.find_closest_number_divisible_by_m(s, 2 ** n_levels, 'higher') for s in input_shape
        ]
        min_pad = [
            utils.find_closest_number_divisible_by_m(s, 2 ** n_levels, 'higher') for s in min_pad
        ]
        pad_shape = list(np.maximum(pad_shape, min_pad))
...
def build_model(path_model_segmentation,
    ...
    if robust:
        ...
        if sigma_smoothing > 0:
            last_tensor = net.output
            set_keras_shape(last_tensor, tensor_static_shape(last_tensor))
    ...
    # add CNN regressor for automated QC — built as a separate model (not chained to U-Net)
    qc_net = None
    qc_prep = None
    if do_qc:
        n_labels_qc = len(np.unique(labels_qc))
        qc_prep = build_qc_prep_model(
            n_labels_seg=n_labels_seg,
            labels_segmentation=labels_segmentation,
            labels_qc=labels_qc,
            input_shape_qc=input_shape_qc,
            n_labels_qc=n_labels_qc,
        )
        qc_net = build_qc_network(
            path_model_qc=path_model_qc,
            input_shape_qc=input_shape_qc,
            n_labels_qc=n_labels_qc,
        )

    return SynthSegModelBundle(seg_net=net, qc_net=qc_net, qc_prep=qc_prep)


def build_qc_prep_model(n_labels_seg, labels_segmentation, labels_qc, input_shape_qc, n_labels_qc):
    shape_prediction = KL.Input([3], dtype='int32', name='qc_shape_input')
    hard_labels = KL.Input([None, None, None], dtype='int32', name='qc_hard_labels')
    last_tensor = MakeShape(input_shape_qc)([hard_labels, shape_prediction])
    last_tensor = layers.ConvertLabels(np.arange(n_labels_seg), labels_segmentation)(last_tensor)
    ...
    return Model(inputs=[hard_labels, shape_prediction], outputs=last_tensor, name='qc_prep')


def build_qc_network(path_model_qc, input_shape_qc, n_labels_qc):
    qc_volume = KL.Input(
        shape=[input_shape_qc, input_shape_qc, input_shape_qc, n_labels_qc],
        name='qc_volume_input',
    )
    pass_model = Model(inputs=qc_volume, outputs=qc_volume, name='qc_pass')
    net = nrn_models.conv_enc(
        input_model=pass_model,
        input_shape=[input_shape_qc, input_shape_qc, input_shape_qc, n_labels_qc],
        nb_levels=4,
        nb_conv_per_level=2,
        conv_size=5,
        nb_features=24,
    ...
    qc_model = Model(inputs=net.inputs, outputs=last_tensor, name='qc_regressor')
    qc_model.load_weights(path_model_qc, by_name=True)
    return qc_model
...
def postprocess(post_patch_seg, post_patch_parc, shape, pad_idx, crop_idx,
    ...
    posteriors = posteriors.astype(np.float16, copy=False)
