...
This module provides:
  ...
  fs_to_hemi(fs_arr, tissue_arr, affine=None)  →  SEG_HEMI  (10-class, uint8)
...
def _assign_unlateralised_wm(hemi, unlat, affine):
    """Split bilateral WM-hypointensities (FS 77) into WM-L / WM-R.

    With an affine, voxels are assigned by RAS-X (x >= 0 → right).
    Without an affine, voxel axis 0 is split at the volume midplane.
    """
    if not np.any(unlat):
        return

    ijk = np.column_stack(np.nonzero(unlat))
    if affine is not None:
        aff = np.asarray(affine, dtype=np.float64)
        ras_x = ijk.astype(np.float64) @ aff[:3, :3].T + aff[:3, 3]
        right = ras_x[:, 0] >= 0.0
    else:
        mid = unlat.shape[0] / 2.0
        right = ijk[:, 0] >= mid

    left_ijk = ijk[~right]
    right_ijk = ijk[right]
    if left_ijk.size:
        hemi[tuple(left_ijk.T)] = 4
    if right_ijk.size:
        hemi[tuple(right_ijk.T)] = 5


def fs_to_hemi(fs_arr, tissue_arr, affine=None):
    """
    ...
    Neutral WM-hypointensities (FreeSurfer label 77) have no L/R tag.
    They are split by RAS-X when ``affine`` is provided, otherwise by
    the midplane of voxel axis 0.

    Parameters
    ----------
    fs_arr     : np.ndarray, integer dtype  -- raw SynthSeg FreeSurfer labels.
    tissue_arr : np.ndarray, uint8          -- output of fs_to_tissue().
    affine     : array-like, optional       -- 4x4 voxel-to-RAS affine.
    ...
    # Remaining WM (FS 77 WM-hypointensities) has no native laterality
    unlat = (tissue_arr == 3) & (hemi == 0) & ~cc_mask
    _assign_unlateralised_wm(hemi, unlat, affine)
