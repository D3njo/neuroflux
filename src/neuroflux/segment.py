...
from neuroflux.paths import resolve_models_dir

# Acceleration policy (configured before TF import in _configure_tf)
_ACCEL_NO_ACCEL = False
...
def _configure_tf(threads: int, low_memory: bool = False, no_accel: bool = False):
    """
    ...
    """
    global _TF_CONFIGURED, _ACCEL_NO_ACCEL
    if _TF_CONFIGURED:
        return

    _ACCEL_NO_ACCEL = no_accel
    ...
    from neuroflux.accel import configure_acceleration

    configure_acceleration(no_accel=no_accel, tf_module=tf)
...
def _check_models(robust: bool):
    ...
    model_dir = resolve_models_dir(require_weights=True)
    missing, corrupt = [], []
    for fname in needed:
        p = model_dir / fname
        if not p.is_file():
...
def _run_synthseg(
    ...
    low_memory mode reduces peak RAM by:
      ...
      - per-axis brain crop (AP=192mm, LR/SI=160mm)
    ...
    if low_memory:
        ...
        _emit("synthseg", 8,
              f"SynthSeg 2.0 ({mode}, low_memory: fast+crop{cropping}+no-QC+no-posteriors, {threads} thread(s))…")
    else:
        cropping      = None
        do_qc_path    = qc_path
    ...
    try:
        _ss_predict(
            ...
            path_model_segmentation   = str(model_dir / (
            ...
            path_model_parcellation   = str(model_dir / "synthseg_parc_2.0.h5"),
            labels_parcellation       = str(_DATA_DIR / "synthseg_parcellation_labels.npy"),
            path_qc_scores            = do_qc_path,
            path_model_qc             = str(model_dir / "synthseg_qc_2.0.h5"),
        ...
        raise RuntimeError(f"SynthSeg inference failed (exit {e.code})") from e
...
def _remap_and_split(fs_seg_path, resampled_path, output_dir, input_path=None):
    ...
    hemi_arr   = fs_to_hemi(fs_arr, tissue_arr, affine=affine)
...
def run_pipeline(
    ...
    low_memory : bool       Fast mode, no QC, no posteriors file, brain crop.
                            Recommended on 6–8 GB systems.
    ...
    _configure_tf(threads, low_memory=low_memory, no_accel=no_accel)
...
def _build_parser():
    ...
    p.add_argument("--low-memory", action="store_true",
                   help="Reduce RAM: fast mode, no QC, no posteriors file, "
                        "and a per-axis brain crop (recommended on 6–8 GB systems)")
    p.add_argument("--no-accel", action="store_true",
                   help="Disable XLA / mixed-precision acceleration (eager FP32 only)")
