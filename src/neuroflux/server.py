...
GET  /gpu
     Checks whether the current Python interpreter's TensorFlow can see a GPU.
...
POST /segment
     ...
     Output is always written to output/segmentation/<stem>/.
     Returns: {"job_id": "<uuid>", "output_dir": "..."}

GET  /status/<job_id>
     Server-Sent Events stream (single consumer per job_id).
...
GET  /job_status/<job_id>
     Non-SSE JSON poll: {"status": "...", "outputs": {...}, "msg": "..."}

GET  /file?path=<absolute_path>
     Serves any output NIfTI file back to the browser as a binary blob.
     Only paths inside the designated output / temp directories are allowed.

Usage
-----
    neuroflux-setup              # one-time model-weight download
    neuroflux-server [--port 5050]
...
_UPLOAD_DIR    = os.path.join(tempfile.gettempdir(), "neuroflux_uploads")
_FOV_DIR       = os.path.join(tempfile.gettempdir(), "neuroflux_fov")
...
_TEMP_TTL_SECONDS = 86400  # 24 hours
...
def _cleanup_temp_dirs():
    """Delete stale upload / FOV-crop temps older than _TEMP_TTL_SECONDS."""
    cutoff = time.time() - _TEMP_TTL_SECONDS
    for root in (_UPLOAD_DIR, _FOV_DIR):
...
def _kill_timed_out_jobs():
    """Terminate running jobs that have exceeded _JOB_TIMEOUT_SECONDS."""
    now = time.time()
    with _jobs_lock:
        timed_out = [
            (jid, j) for jid, j in _jobs.items()
            if j.get("status") == "running"
            and now - j.get("created", 0) > _JOB_TIMEOUT_SECONDS
        ]
    for jid, job in timed_out:
        proc = job.get("proc")
        if proc is not None and proc.poll() is None:
            try:
                ...
                pass
        _push_job_error(job, "job timed out")


def _cleanup_old_jobs():
    """Remove finished jobs older than _JOB_TTL_SECONDS; kill timed-out runners."""
    _cleanup_temp_dirs()
    _kill_timed_out_jobs()
    now = time.time()
    with _jobs_lock:
...
def _allowed_root(path: str, *, suffixes=None, must_exist=True) -> str | None:
    """
    Return the real path if it lives under _OUTPUT_DIR or the system temp dir.

    Optional ``suffixes`` is a tuple of allowed endings (checked after realpath).
    Trailing os.sep is required so /tmp/neuroflux does not match /tmp/neuroflux_evil.
    """
    ...
    if must_exist and not os.path.isfile(real):
        return None
    if suffixes and not any(real.endswith(s) for s in suffixes):
        return None
    if not _under_allowed_root(real):
        return None
    return real


def _safe_path(path: str) -> bool:
    """NIfTI files under allowed roots — used by GET /file."""
    return _allowed_root(path, suffixes=_NIFTI_SUFFIXES) is not None
...
def _push_job_error(job: dict, msg: str) -> None:
    job["status"] = "error"
    job["msg"] = msg
    q = job.get("queue")
    if q is not None:
        try:
            q.put(json.dumps({"status": "error", "msg": msg}))
            q.put(None)
        except Exception:
            pass
...
def gpu_check():
    """
    Check whether the current interpreter's TensorFlow can see a GPU.
    ...
    result = {"gpu": False, "name": None, "note": None, "accel": "eager", "dtype": "float32"}
    probe = (
        "import json; "
        "import os; "
        "os.environ.setdefault('KERAS_BACKEND', 'tensorflow'); "
        "import tensorflow as tf; "
        "from neuroflux.accel import configure_acceleration, get_policy; "
        "configure_acceleration(no_accel=False, tf_module=tf); "
        "gpus = tf.config.list_physical_devices('GPU'); "
        "n = gpus[0].name.split('/')[-1] if gpus else None; "
        "policy = get_policy(); "
        "print(json.dumps({"
        "'gpu': bool(gpus), 'count': len(gpus), 'name': n, "
        "'accel': policy.accel_mode, 'dtype': policy.compute_dtype, "
        "'accelerator': policy.kind.value, 'bucket_padding': policy.use_bucket_padding"
        "}))"
    )
    try:
        r = subprocess.run(
        ...
        if r.returncode == 0 and r.stdout.strip():
            ...
            if not result.get("gpu"):
                if platform.system() == "Windows":
                    result["note"] = (
                        "CPU mode — TensorFlow 2.18 can use CUDA on Windows. "
                        "Install a CUDA-enabled TF build, or use WSL2+CUDA."
                    )
                else:
                    result["note"] = "CPU mode — no GPU visible to TensorFlow."
        else:
            err = (r.stderr or r.stdout or "unknown error").strip()[:200]
            result["note"] = f"GPU probe failed: {err}"
    except subprocess.TimeoutExpired:
...
def serve_json():
    ...
    real = _allowed_root(path, suffixes=_JSON_SUFFIXES)
    if real is None:
...
def upload():
    ...
    save_path = os.path.join(_UPLOAD_DIR, f"{uuid.uuid4().hex}_{fname}")
    f.save(save_path)
...
def analyze_fov():
    ...
    if _allowed_root(path, suffixes=_NIFTI_SUFFIXES) is None:
        return jsonify({"error": f"File not found: {path}"}), 400

    from neuroflux.segment import analyze_and_crop_fov

    try:
        result = analyze_and_crop_fov(path, _FOV_DIR, margin_mm=margin)
    except Exception as e:
...
def manual_crop():
    ...
    os.makedirs(_FOV_DIR, exist_ok=True)
    stem     = os.path.splitext(os.path.basename(path))[0].removesuffix(".nii")
    out_path = os.path.join(_FOV_DIR, f"{stem}_manual_crop.nii.gz")
...
def segment():
    ...
    # Client-supplied output_dir is ignored — always write under output/segmentation/<stem>/
    ...
    threads = min(threads, os.cpu_count() or 8)

    if not input_path:
        return jsonify({"error": "input_path is required"}), 400
    if _allowed_root(input_path, suffixes=_NIFTI_SUFFIXES) is None:
    ...
    if _running_job_count() >= _MAX_CONCURRENT_JOBS:
        return jsonify({"error": "a segmentation job is already running"}), 429
...
def cancel_job(job_id: str):
    ...
    if proc and proc.poll() is None:
        try:
            ...
            pass
        with _jobs_lock:
            _push_job_error(_jobs[job_id], "cancelled")
...
def status_stream(job_id: str):
    with _jobs_lock:
        ...
        if job.get("sse_attached"):
            return jsonify({"error": "SSE already attached for this job"}), 409
        job["sse_attached"] = True
...
def get_mask():
    ...
    if _allowed_root(seg_path, suffixes=_NIFTI_SUFFIXES) is None:
...
def refine():
    ...
    for p in [seg_path, nifti_path]:
        if _allowed_root(p, suffixes=_NIFTI_SUFFIXES) is None:
...
def serve_stl_file():
    ...
    if not path:
        abort(400, "path required")
    real = _allowed_root(path, suffixes=_STL_SUFFIXES)
    if real is None:
        abort(403, "access denied or file not found")
    return send_file(real, mimetype="model/stl")
...
def save_file():
    ...
    """
    filename = os.path.basename(request.args.get("filename", "file.nii") or "file.nii")
    stem     = request.args.get("stem", "ct_bone") or "ct_bone"
    ...
    if len(request.data) > app.config.get("MAX_CONTENT_LENGTH", 500 * 1024 * 1024):
        return jsonify({"error": "payload too large"}), 413
    if use_temp:
        os.makedirs(_UPLOAD_DIR, exist_ok=True)
        out_path = os.path.join(_UPLOAD_DIR, f"{uuid.uuid4().hex}_{filename}")
        with open(out_path, "wb") as f:
            f.write(request.data)
        return jsonify({"path": out_path, "filename": filename, "temp": True})
    out_dir  = os.path.join(_OUTPUT_DIR, "3d-files", stem)
    os.makedirs(out_dir, exist_ok=True)
    ...
    with open(real_out, "wb") as f:
        f.write(request.data)
    return jsonify({"path": real_out, "filename": filename, "temp": False})


@app.delete("/delete_file")
def delete_file():
    """Delete a file under output/ or the system temp dir (STL-export cleanup)."""
    path = (request.get_json(force=True, silent=True) or {}).get("path", "")
    real = _allowed_root(path, must_exist=True)
    if real is None:
...
def ct_bone():
    ...
    if _allowed_root(ct_path, suffixes=_NIFTI_SUFFIXES) is None:
...
def dicom_series():
    ...
    if ".." in os.path.normpath(folder).split(os.sep) or not os.path.isdir(folder):
        return jsonify({"error": "folder not found"}), 400
...
def dicom_convert_path():
    ...
    uid    = str(body.get("series_uid") or body.get("uid") or "").strip()
    if not folder:
        return jsonify({"error": "folder not found"}), 400
    folder = os.path.realpath(folder)
    ...
    try:
        if uid:
            import pydicom
            tmp_series = tempfile.mkdtemp(prefix="nfx_dcm_series_")
            copied = 0
            for root, dirs, files in os.walk(folder):
                ...
                for fname in files:
                    ...
                    try:
                        ...
                        continue
                    if str(getattr(ds, "SeriesInstanceUID", "") or "") != uid:
                        continue
                    dest_name = f"{copied:05d}_{os.path.basename(fname)}"
                    shutil.copy2(fpath, os.path.join(tmp_series, dest_name))
                    copied += 1
            if copied == 0:
                return jsonify({"error": "no DICOM slices matched series_uid"}), 400
            convert_src = tmp_series

        dicom2nifti.convert_directory(convert_src, out_dir, compression=True, reorient=True)
        nii_files = sorted(
        ...
        if tmp_series:
            shutil.rmtree(tmp_series, ignore_errors=True)
...
def dicom_convert():
    ...
    out_dir = os.path.join(_OUTPUT_DIR, "ct_converted", "upload_" + uuid.uuid4().hex[:12])
    os.makedirs(out_dir, exist_ok=True)
    try:
        for f in files:
        ...
        try:
            ...
            dicom2nifti.convert_directory(
                tmp_in, out_dir, compression=True, reorient=True
            )
        except Exception as e:
            return jsonify({"error": f"DICOM conversion failed: {e}"}), 500
...
def main():
    ...
    if not loopback:
        print(
            f"\n  WARNING: binding to {args.host} exposes this API on the network.\n"
            "  CORS remains restricted to localhost origins. Do not expose this\n"
