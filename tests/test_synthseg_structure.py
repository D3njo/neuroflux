...
class TestNoStandaloneKeras:
    ...
    def test_neuron_models_disables_fused_batch_norm(self):
        ...
        assert "fused=False" in text, (
            "3D SynthSeg models must try fused=False first, then fall back for Keras 3"
        )
...
class TestSegmentModule:
    ...
    def test_sets_keras_backend_before_tf(self):
        text = self.SEG.read_text(encoding="utf-8")
        assert 'KERAS_BACKEND' in text
        assert "reset_default_graph" not in text
        assert "configure_acceleration" in text
        assert "--no-accel" in text
